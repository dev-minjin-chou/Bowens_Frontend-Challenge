...
function App() {
  ...
  //Get current movies.
  const lastMovies = currentPage * moviesPerPage;
  const firstMovies = lastMovies - moviesPerPage;
  const currentMovies = movies.slice(firstMovies, lastMovies);
  ...
  const filterMovies = currentMovies.filter(
    (movie) =>
      ...
      <div className="website">
        ...
        <div className="movie-container">
          {filterMovies.length > 0 ? (
            filterMovies.map((movie) => <Movie key={movie.id} movie={movie} />)
          ) : (
            <div>
